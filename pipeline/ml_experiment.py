...
# read the data file
df = pd.read_csv("filter_and_clean.csv")
...
def create_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # delete delays > 1h
    # Convert to float64 first to avoid dtype warnings
    df2 = df2.copy()
    df2['DEPARTURE_DELAY'] = df2['DEPARTURE_DELAY'].astype('float64')
...
# MLflow Projects will handle experiment management automatically
run_name = "Training Phase"
...
with mlflow.start_run(nested=True, run_name=run_name):
    score_min = 10000
    alpha_max = num_alpha_increments * 2
    count = 1
...
with mlflow.start_run(nested=True, run_name="Final Model - Test Data"):
    # Log the informational log files
    mlflow.log_artifact(logname, "logs") # Use the logname variable
    mlflow.log_artifact("airport_encodings.json", "artifacts") # Also good to log this

    # Log the input parameters to the final regression
    mlflow.log_param("num_alpha_increments", num_alpha_increments)
    mlflow.log_param("polynomial_order", order)
    mlflow.log_param("ridge_alpha", parameters[0]/10)

    # Log the performance plot (already saved on line 352)
    ...
    print(mlflow.log_metric("Test Data Average Delay min", np.sqrt(score)))

    # Log the final model as an artifact
    mlflow.sklearn.log_model(sk_model=ridgereg, name="final_model", input_example=X_[:5], signature=mlflow.models.infer_signature(X_, result))
